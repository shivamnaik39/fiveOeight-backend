...
def update_data(data):
    ...
    for filepath, value in data["img_alt"].items():
        ...
        # modified_path = ntpath.normpath(modified_path)
        print(modified_path)
...
def get_modified_path(filepath):
    # Split the filepath by the correct separator to get individual directories and filename
    directories, filename = os.path.split(filepath)

    # Add the "downloads" directory at the beginning of the directories list
    directories = ["downloads"] + directories.split(os.sep)
    # Modify the first directory to add "_modified" suffix
    directories[1] = directories[1] + "_modified"

    # Join the directories list and filename using the correct separator to form the final filepath
    modified_filepath = os.path.join(*directories) + os.sep + filename
